...
const commandsToTest = [
  ...
  { desc: "Create a test device block (Sign Out - NOT STARTED)", cmd: `-add-device-block "Raycast_Test_SignOut" -sign-out` },
  { desc: "Create a test device block (Shut Down - NOT STARTED)", cmd: `-add-device-block "Raycast_Test_ShutDown" -shut-down` },
...
for (const item of commandsToTest) {
  ...
  try {
    ...
    const formattedOutput = output.trim() ? output.trim() : "(Command executed successfully, but returned no text output)";
